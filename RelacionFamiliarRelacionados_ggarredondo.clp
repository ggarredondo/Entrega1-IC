...
;;;;; Solicitamos el nombre de la persona sobre el que se desea información, guardamos el nombre y añadimos el hecho
 
(defrule pregunta 
=>
   (printout t "Dime el nombre de la persona de la Familia Castro sobre la que quieres informacion (escribe solo el nombre): " crlf)
   (assert (personapedida (read))) )
   
;;;;; Solicitamos la relación con la persona sobre la que se quiere conocer los miembros, guardamos la relación y añadimos el hecho
 
(defrule pregunta2
(personapedida ?nombre)
=>
   (printout t "Dime la relacion con " ?nombre " sobre la que quieres saber los miembros (en mayusculas): " crlf)
   (assert (relacionpedida (read))) )
   
;;;;; Dado el nombre de la persona y la relación pedidas, se imprime por pantalla todos los hombres que tienen esa relación con la persona en cuestión
;;;;; Para ello se buscan todos los sujetos que cumplen esa relación, tomando el nombre de la persona como objeto, y después comprobando si esos sujetos son hombre

(defrule relacionadosEncontradosMasculino
	(personapedida ?nombre)
	(relacionpedida ?relacion)
	(Relacion (tipo ?relacion) (sujeto ?s) (objeto ?nombre))
	(hombre ?s)
=>
	(printout t ?s " es " ?relacion " de " ?nombre crlf) )

;;;;; Hacemos lo mismo que en la regla anterior pero para las mujeres
;;;;; Tras comprobar que los sujetos son mujer, utilizamos el hecho de "femenino" para obtener el nombre de la relación en corcondancia con el género

(defrule relacionadosEncontradosFemenino
	(personapedida ?nombre)
	(relacionpedida ?relacion)
	(Relacion (tipo ?relacion) (sujeto ?s) (objeto ?nombre))
	(mujer ?s)
	(femenino ?relacion ?r)
=>
	(printout t ?s " es " ?r " de " ?nombre crlf) )

;;;;; Utilizando el operador not, esta última regla se dispara si no existe ningún sujeto para el cual se cumpla la relación pedida con la persona
;;;;; Se imprime por pantalla el aviso de que la persona no tiene familiares con esa relación

(defrule relacionadosNoEncontrados
	(personapedida ?nombre)
	(relacionpedida ?relacion)
	(not (Relacion (tipo ?relacion) (sujeto ?s) (objeto ?nombre)))
=>
	(printout t ?nombre " no tiene " ?relacion "S " crlf) )
